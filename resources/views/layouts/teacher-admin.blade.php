        ...
        <aside class="main-sidebar sidebar-dark-primary elevation-4">
            ...
            <div class="sidebar">
               ...
               with font-awesome or any other icon font library -->
                        ...
                        <li class="nav-item">
                            <a href="{{ route('daftar-guru') }}" class="nav-link active">
                                <i class="nav-icon fas fa-table"></i>
                                <p>
                                    Daftar Guru
                                </p>
    ...
    <script>
        $(function(){
                $('.btn-edit-guru').on('click', function(){
                    let id = $(this).data('id');
                        $.ajax({
                        url: `/daftar-guru/edit/${id}/show`,
                        method: 'GET',
                        success: function(data){
                            var parse_data = jQuery.parseJSON(JSON.stringify(data.data[0]));
                            $('#action-edit').attr('action', `/daftar-guru/edit/${parse_data.id}/update`);
                            $('#name-edit').val(parse_data.name);
                            $('#nip-edit').val(parse_data.nip);
                            if(parse_data.gender=='L'){
                            $('#gender-l-edit').prop('checked',true);
                            }else{
                            $('#gender-p-edit').prop('checked',true);
                            }
                            $('#course-id-edit').val(`${parse_data.courses_id}`).trigger('change.select2');
                        }
                    });
                });
            });
    ...
    <script>
        $(function(){
                $('.btn-hapus-guru').on('click', function(){
                    let id = $(this).data('id');
                        $('#action-hapus').attr('action', `/daftar-guru/${id}`);
                });
            });
    </script>
</body>
