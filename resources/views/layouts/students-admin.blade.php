        ...
        <aside class="main-sidebar sidebar-dark-primary elevation-4">
            ...
            <div class="sidebar">
               ...
               with font-awesome or any other icon font library -->
                        ...
                        <li class="nav-item">
                            <a href="{{ route('daftar-siswa') }}" class="nav-link active">
    ...
    <script>
        $(function(){
            $('.btn-edit-siswa').on('click', function(){
                let id = $(this).data('id');
                    $.ajax({
                    url: `/daftar-siswa/edit/${id}/show`,
                    method: 'GET',
                    success: function(data){
                        var parse_data = jQuery.parseJSON(JSON.stringify(data.data[0]));
                        $('#action-edit').attr('action', `/daftar-siswa/edit/${parse_data.id}/update`);
                        $('#name-edit').val(parse_data.name);
                        $('#nisn-edit').val(parse_data.nisn);
                        $('#grade-edit').val(parse_data.grade);
                        $('#majors-edit').val(parse_data.majors);
                        if(parse_data.gender=='L'){
                            $('#gender-l-edit').prop('checked',true);
                        }else{
                            $('#gender-p-edit').prop('checked',true);
                        }
                        $('#date-edit').val(parse_data.birthdate);
                    }
                });
            });
        });
